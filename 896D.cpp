...
template <typename T> T CompModInverse(T p, T CompMod)
{
    return BigMod(p,phi(CompMod)-1);
}
...
void init(){
    ...
    for(int i=2; i<mx; i++){
        ...
        primePower[i][j]=primePower[i-1][j];
        while(x%prime[j]==0) {x/=prime[j]; primePower[i][j]++;}
        }
        fact[i]=fact[i-1]*x%m;
        inv[i] =BigMod(fact[i],phim-1,m);
        }
    }
...
ll nCr(ll k, ll r){
    if(r<0 || r>k ) return 0;
    if(r==0) return 1;
    ll ans = (fact[k]*inv[r]%m)*inv[k-r]%m;
    for(int i=0; i<prime.size(); i++){
        ans= ans*BigMod(prime[i],primePower[k][i]-primePower[r][i]-primePower[k-r][i],m)%m;
    }
    ans%=m;
    return ans;
}
    ...
    int main()
    {
        ...
        for(ll i=0; i<=n; i++){
            ll p = n-i;
            ans = (ans + (nCr(n,i)*((nCr(p,(p+l+1)/2)-nCr(p,(p+min(r,p))/2+1)+m)%m)%m))%m;
        }
        cout << ans ;
    }