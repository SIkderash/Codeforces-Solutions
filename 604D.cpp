...
const ld eps        = 1e-9;
ll fact[mx];
void factorial(){fact[0]=1;loop1(i,mx-1) fact[i]=(fact[i-1]*i);}
ll nCr(ll n,ll r){return (fact[n]*ModInverse(((ll)fact[n-r]*(ll)fact[r]),(ll)mod));}
...
///code start here
    int main()
    {
        ll p, k;
        cin >> p >> k;
        if(k==0) cout << BigMod(p,p-1,(ll)mod);
        else if(k==1) cout << BigMod(p,p,(ll)mod);
        else {
                ll m;
                for( m=1; m<=p-1; m++){
                    if(BigMod(k,m,p)==1) break;
                }
                cout << BigMod(p,(p-1)/m,(ll)mod);
        }
