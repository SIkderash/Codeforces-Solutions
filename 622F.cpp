...
///code start here
    ...
    int main()
    {
        ...
        if(n<v.size()) cout << v[n];
            ...
            ll ans1 = 1, ans = 0;
            for(int i=1; i<=k+1; i++){
                ans1 = ans1*((((n-i)*ModInverse(-1*1LL*i,(ll)mod))%mod+mod)%mod)%mod;
            }
            for(int i=0; i<=k+1; i++){
                ans+= ans1*v[i] %mod;
                ans1 = ((ans1*((n-i)*ModInverse((ll)n-i-1, (ll)mod)%mod)%mod)+mod)%mod;
                ans1 = ((ans1*((i-k-1)*ModInverse((ll)i+1, (ll)mod)%mod)%mod)+mod)%mod;
            }
             cout << ans%mod ;
        }
