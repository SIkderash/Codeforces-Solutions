...
int main()
{
    ...
    int size1 = R, size2 = G, size3 = B;
    while((size1>=1 && size2>=1) || (size2>=1 && size3>=1) || (size1>=1 && size3>=1)){
        int flag1 = 0, flag2=0, flag3=0;
        if(size1==1) flag1++;
        if(size2==1) flag2++;
        if(size3==1) flag3++;
        if(flag1+flag2+flag3==2 && size1+size2+size3>=4){
            if(flag1==1 && flag2==1){
                sum+= max(r[size1-1]*g[size2-1], max(r[size1-1]*b[size3-1]+g[size2-1]*b[size3-2], r[size1-1]*b[size3-2]+g[size2-1]*b[size3-1]));
                break;
            }
            else if(flag1==1 && flag3==1){
                sum+= max(r[size1-1]*b[size3-1], max(r[size1-1]*g[size2-1]+b[size3-1]*g[size2-2], r[size1-1]*g[size2-2]+b[size3-1]*g[size2-1]));
                break;
            }
            else{
                sum+= max(g[size2-1]*b[size3-1], max(g[size2-1]*r[size1-1]+b[size3-1]*r[size1-2], g[size2-1]*r[size1-2]+b[size3-1]*r[size1-1]));
                break;
            }
        }

        vector<ll>ans;
        if(size1>=1) ans.pb(r[size1-1]);
        if(size2>=1) ans.pb(g[size2-1]);
        if(size3>=1) ans.pb(b[size3-1]);
        sort(ans.begin(), ans.end());
        sum+=ans[ans.size()-1]*ans[ans.size()-2];
        //cout << ans.size()-1 <<" "<<ans.size()-2 <<endl;
        int del = 0;
        if(size1>=1){
            if(ans[ans.size()-1] == r[size1-1] || ans[ans.size()-2] ==  r[size1-1]) {size1--; del++;}
        }
        if(del==0) {size2--; size3--; continue;}
        if(size2>=1){
            if(ans[ans.size()-1] == g[size2-1] || ans[ans.size()-2] ==  g[size2-1]) {size2--; del++;}
        }
        if(del<2 && size3>=1){
            if(ans[ans.size()-1] == b[size3-1] || ans[ans.size()-2] ==  b[size3-1]) {size3--; del++;}
        }
    }
    cout << sum ;
    return 0;
