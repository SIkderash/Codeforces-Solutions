...
short n, m;
int k;
vector<short> g[3000+5];
set<short> st[3005][3005];
short d[3005][3005];
short p[3005][3005];
void bfs(short x, short y){
    ...
    while(!q.empty()){
        ...
        q.pop();
        for(auto y : g[u.S]){
            if(p[u.S][y]!= -1 || st[u.F][u.S].count(y)) continue;
            p[u.S][y] = u.F;
            q.push(make_pair(u.S,y));
            d[u.S][y] = d[u.F][u.S]+1;
        }
    }
}
int main()
{
    ...
    short dis = SHRT_MAX;
    for(int i=0; i<n; i++){
        if(d[i][n-1]<dis) {dis = d[i][n-1]; save = i;}
    }
    if(dis==SHRT_MAX) {cout << -1; return 0;}
    cout << dis << endl;
    vector<int>ans;
    pair<short,short> cur = {save,n-1};
    while(cur.F!=-1){
        ans.pb(cur.S);
        short temp = cur.F;
        cur.F = p[cur.F][cur.S];
