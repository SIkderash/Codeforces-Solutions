...
//ll fact[mx];
    ...
    {
        fast;
        ll n, k, num=1, denom, d = 2, legend = 0; cin>> n >> k;
        if(n<=62 && (1ll<<n)<k) {cout << "1 1"; return 0;}
        denom = BigMod(2ll,(n%(mod-1)*((k-1)%(mod-1))%(mod-1)),(ll)mod);
        ll tmp = k-1;
        while(tmp) legend += tmp/2, tmp/=2;
        denom = denom*ModInverse(BigMod(2ll,legend%(mod-1),(ll)mod),(ll)mod)%mod;
        if(k-1>=mod) {cout << denom<<" "<<denom; return 0;}
        ll p2n = BigMod(2LL,n%(mod-1),(ll)mod);
        for(ll i = 1; i<=k-1; i++)
            num = (num*(p2n - i))%mod;
        num = num*ModInverse(BigMod(2ll,legend%(mod-1),(ll)mod),(ll)mod)%mod;
        num = (denom - num + mod)%mod;
        cout << num << " " << denom;
    }