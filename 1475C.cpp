# include <bits/stdc++.h>
    ...
    ///code start here
int cnt[2*100000+5], cnt1[2*100000+5];
int main(){
    fast;
    int t;
    cin >> t;
    while(t--){
        ...
        for(int i=0; i<2*100000+5; i++) {cnt[i] =0; cnt1[i]=0;}
        ...
        ll ans =0;
        //for(int i=0; i<10; i++) {cout << i << " "<< cnt[i] << nl; /*cout <<i << " " <<  cnt1[i] << nl;*/}
        for(int i=0; i<k; i++){
            //cout << cnt[a[i]] << " " << cnt1[b[i]] << nl;
            ans+=k-(cnt[a[i]]-1)-(cnt1[b[i]]-1)-1;
        }
        ans/=2;
        cout << ans << nl;
    }
