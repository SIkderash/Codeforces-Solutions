# include <bits/stdc++.h>
    ...
    const ld eps        = 1e-9;
    ll fact[1005+5];
    void factorial(){fact[0]=1;loop1(i,1005) fact[i]=(fact[i-1]*i)%mod;}
...
int main(){
   ...
   if(n==1) {cout << b[0] << nl; return 0;}
   ll prefix[n], suffix[n], ans = 0;
   prefix[0] = a[0];
   suffix[n-1] = a[n-1];
   ...
   for(int i=0; i<n; i++){
       ll temp_OR;
        if(i==0) temp_OR =  b[i]|suffix[i+1];
        else if(i==n-1) temp_OR =  prefix[i-1]|b[i];
        else  temp_OR = prefix[i-1]|b[i]|suffix[i+1];
        ans = max(ans, temp_OR);
   }
   cout << ans << nl;
